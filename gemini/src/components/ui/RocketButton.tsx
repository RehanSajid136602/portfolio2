...
export function RocketButton({ email }: RocketButtonProps) {
  ...
  const handleLaunch = async () => {
    ...
    try {
      ...
      // Confetti: dynamically import to avoid adding to initial client bundle
      try {
  ...
  return (
    <button
      ...
      className="group relative flex items-center gap-3 rounded-full bg-white px-4 py-3 text-xs sm:text-sm md:text-base font-semibold text-black transition-all hover:bg-zinc-200 disabled:cursor-not-allowed w-full sm:w-auto sm:min-w-[240px] justify-center max-w-full overflow-hidden"
    >
      <span className={status === "success" ? "text-emerald-600 flex items-center gap-2 truncate" : "flex items-center gap-2 truncate"}>
        {status === "success" ? (
          <>
            <Check size={20} />
            Email Copied!
